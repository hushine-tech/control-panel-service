...
// to talk to account-service. Account-service still owns users; market-data
...
// Client wraps the account-service gRPC stub.
type Client struct {
	conn *grpc.ClientConn
	cli  accountv1.AccountServiceClient
}

// New dials account-service at addr. Outbound interceptors (logging,
...
func New(addr string, opts ...grpc.DialOption) (*Client, error) {
	if addr == "" {
		return nil, fmt.Errorf("account-service grpc address is empty")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("dial account-service: %w", err)
	}
	return &Client{
		conn: conn,
...
func (c *Client) GetUserPlanCode(ctx context.Context, userID int64) (string, error) {
	...
	if resp == nil || resp.GetUser() == nil {
		return "", fmt.Errorf("account-service returned empty user")
	}
